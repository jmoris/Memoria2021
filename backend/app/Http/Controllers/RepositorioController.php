    ...
    public function getAnalyticsReports(Request $request){
        try{
            ...
            if(!is_dir($path_project)){
                ...
                $str = 'cd '.$path.' && sudo git clone https://'.$user->gh_user.':'.$user->gh_token.'@github.com/'.$nombre[0].'/'.$nombre[1].'.git && sudo git -c ./'.$nombre[1].' log --pretty=format:"[%h] %an %ad %s" --date=short --numstat --before='.date('Y-m-d').' > '.$fileNames[0].' && sudo java -jar code-maat-0.8.5-standalone.jar maat -l '.$fileNames[0].' -c git -a summary > '.$fileNames[1].' && sudo perl cloc ./'.$nombre[1].' --by-file --csv --quiet --report-file='.$fileNames[2].' && sudo java -jar code-maat-0.8.5-standalone.jar maat -l '.$fileNames[0].' -c git -a revisions > '.$fileNames[3].' && sudo python2.7 scripts/merge_comp_freqs.py '.$fileNames[3].' '.$fileNames[2].' > '.$fileNames[4];
                $res = shell_exec($str);
                Log::info($res);
                if($res != ''){
                    $repo_cli = true;
                ...
                $str = 'cd '.$path.' && sudo rm -Rf '.$nombre[1].' && sudo git clone https://'.$user->gh_user.':'.$user->gh_token.'@github.com/'.$nombre[0].'/'.$nombre[1].'.git && sudo git -c ./'.$nombre[1].' log --pretty=format:"[%h] %an %ad %s" --date=short --numstat --before='.date('Y-m-d').' > '.$fileNames[0].' && sudo java -jar code-maat-0.8.5-standalone.jar maat -l '.$fileNames[0].' -c git -a summary > '.$fileNames[1].' && sudo perl cloc ./'.$nombre[1].' --by-file --csv --quiet --report-file='.$fileNames[2].' && sudo java -jar code-maat-0.8.5-standalone.jar maat -l '.$fileNames[0].' -c git -a revisions > '.$fileNames[3].' && sudo python2.7 scripts/merge_comp_freqs.py '.$fileNames[3].' '.$fileNames[2].' > '.$fileNames[4];
                Log::info($str);
