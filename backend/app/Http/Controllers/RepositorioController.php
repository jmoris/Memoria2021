...
class RepositorioController extends Controller
{
    ...
    public function getInfoProyecto(Request $request){
        try{
            ...
            Log::info('cd '.$path_project.' && git rev-list HEAD --count');
...
/*
            if(is_dir($path)){
                $str = 'cd '.$path.' && git pull && git -c ./'.$nombre[1].' log --pretty=format:"[%h] %an %ad %s" --date=short --numstat --before='.date('Y-m-d').' > '.$fileNames[0].' && java -jar code-maat-0.8.5-standalone.jar maat -l '.$fileNames[0].' -c git -a summary > '.$fileNames[1].' && perl cloc ./'.$nombre[1].' --by-file --csv --quiet --report-file='.$fileNames[2].' && java -jar code-maat-0.8.5-standalone.jar maat -l '.$fileNames[0].' -c git -a revisions > '.$fileNames[3].' && /usr/bin/python2.7 scripts/merge_comp_freqs.py '.$fileNames[3].' '.$fileNames[2].' > '.$fileNames[4];
                $res = shell_exec($str);
            }else{
            ...
            $resultado5 = shell_exec('cd '.$path.' && /usr/bin/python2.7 scripts/merge_comp_freqs.py hib_freqs.csv hib_lines.csv > Prueba.csv');*/
    ...
    public function getBranchCommits(Request $request){
        try{
            $user = $request->user();
            $project = Proyecto::findOrFail($request->project_id);
            $repositorio = Repositorio::find($project->repositorio_id);
